...
const cairo = Cairo({   weight: ['400', '700'],
  style: ['normal',],
...
const Profile = ({UserCode} : {UserCode:string}) => {
      ...
      const fetchUser = async () => {
        try {
          ...
          if (data.status === "error" && data.errorCode === 1004) {
            Cookies.remove('scc-code'); // Remove the cookie
          } else {
    ...
    return (
      <Suspense fallback="loading" >
          <span className="">{UserData?.global_name}</span>
      ...
      >
        ...
        <MenuItem>
          <Link
   href="/dashboard?type=servers"            className="block px-4 py-2 text-sm text-white data-[focus]:bg-[#2a2a2a] data-[focus]:outline-none hover:bg-[#2a2a2a] transition-colors"
          >
